...
public:
	...
	TList(const TList& other) {//copy constructor
		if (other.sz != 0) {
			this->sz = other.sz;
			Node* curr1 = other.first; //идем по элементам копируемого списка
			Node* curr2 = new Node(nullptr, curr1->value); //то, куда копируем элементы списка
			this->first = curr2;

			while (!curr1) {//пока элемент существует
				Node* next1 = curr1->next; 
				if (!next1) break; //проверяем есть ли в копируемом списке следующий элемент
				curr2->next = new Node(nullptr, next1->value); //если есть, то копируем следующий элемент
				curr2 = curr2->next; //двигаемся по копируемому списку
				curr1 = curr1->next; //и по тому, куда копируем
			}
		}
	}
